...
class ScriptEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        try:
            ...
            try:
                shutil.move(file_path, working_file)
                # Cập nhật file path trong cả job và context
                job.file_path = working_file
                ...
                # Nếu có lỗi khi di chuyển hoặc xử lý, di chuyển vào error
                error_path = os.path.join(channel_paths["error_dir"], os.path.basename(file_path))
                if os.path.exists(working_file):
                    shutil.move(working_file, error_path)
